...
| Phase 0 — Foundation, Spikes, Privacy Narrative | 0.5 In-keyboard recording spike | 🟩 Done | Harness exists; old-device / borrowed-device hardware pass skipped by product decision. Primary-flow viability remains an explicit Phase 1 / TestFlight risk; fallback flow stays first-class. |
| Phase 0 — Foundation, Spikes, Privacy Narrative | 0.6 Remaining platform spikes | 🟧 In progress | Speech locale and crash-reporting verdicts are done; insert-context, Open Access, and audio-interruption harnesses are in place; host-app insertion, audio manual matrix, and postponed Open Access manual matrix still pending. |
...
- **In-keyboard recording** — microphone + `SFSpeechRecognizer` inside the Keyboard Extension on the required iOS test versions from [`docs/specs/testing.md`](docs/specs/testing.md); measure peak memory, latency, stability over 5 min of repeated 10 s dictations when hardware is available. Harness implemented; old-device / borrowed-device hardware pass skipped by product decision. See [`docs/spikes/in-keyboard-recording.md`](docs/spikes/in-keyboard-recording.md). (See [performance-and-memory](docs/specs/performance-and-memory.md), [speech-and-postprocessing](docs/specs/speech-and-postprocessing.md).)
- **Open Access** — confirm `openURL` and microphone-in-extension behavior with and without Open Access. Harness is wired; manual feature matrix is postponed until after the audio interruption spike. See [`docs/spikes/open-access-behavior.md`](docs/spikes/open-access-behavior.md). (See [privacy-and-app-review](docs/specs/privacy-and-app-review.md).)
...
- **Audio** — interruption tests (call, Siri, Focus, headphone unplug) for both flows. Harness is wired; manual device matrix remains pending. See [`docs/spikes/audio-interruption.md`](docs/spikes/audio-interruption.md).
