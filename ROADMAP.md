...
| Phase 0 — Foundation, Spikes, Privacy Narrative | 0.6 Remaining platform spikes | 🟧 In progress | Speech locale and crash-reporting verdicts are done; insert-context planning and Open Access harnesses are in place; host-app insertion, Open Access manual matrix, and audio interruption verdicts still pending. |
...
- **Open Access** — confirm `openURL` and microphone-in-extension behavior with and without Open Access. Harness is wired; manual feature matrix remains pending. See [`docs/spikes/open-access-behavior.md`](docs/spikes/open-access-behavior.md). (See [privacy-and-app-review](docs/specs/privacy-and-app-review.md).)
